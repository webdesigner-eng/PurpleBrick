...
const projects = [
  ...
  ["P", "Power Play", "D2C Food & Dairy Brand · Ad Creatives & Social Media"],
];
    ...
    <main className="designing-page overflow-hidden bg-[#050719] text-white">
      ...
      <section className="designing-dark-section px-3 py-20 sm:py-28">
        <Container className="common-container">
          ...
          <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-5">{team.map(([initial, name, role]) => <div className="text-center flex flex-col gap-3" key={name}><div className="designing-avatar">{initial}</div><h3 className="mt-4">{name}</h3><p className="mt-1 text-[#abb0c6]">{role}</p></div>)}</div>
        </Container>
      </section>
